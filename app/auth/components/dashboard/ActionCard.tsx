...
export const ActionCard = () => {
  ...
  const [headerValue, setHeaderValue] = useState("Check the responsive layout on all devices")
  return (
    <div className="text-sm p-2 ml-2">
      <div className="bg-white p-2 rounded mt-1 border-b border-grey cursor-pointer hover:bg-grey-lighter">
        {inEditMode ? (
          ...
          <a onClick={() => setShowModal(true)}>{headerValue}</a>
        )}
        {showModal ? <Modal /> : ""}
        <div className="text-grey-darker mt-2 ml-2 flex justify-between items-start">
          ...
          {!inEditMode ? (
            ...
            <BiCheck
              className="m-2 text-xl text-blue-500 hover:text-blue-600"
              onClick={() => {
                setInEditMode(false)
              }}
            />
